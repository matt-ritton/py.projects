...
#Label Widget
label = Label(bg=bg_color, image=img) #Show the image in a label widget
...
def controller(img_number): #Function to control the exhibition of images
    ...
    label = Label(bg=bg_color, image=img_control) #Show the image in a label widget
    ...
    btn_next = Button(root, text=">>", padx=btn_padx, pady=btn_pady, fg=fg_color, bg=bg_color,
                      activebackground=activebg_color, activeforeground=fg_color, font=((button_font)),
                      command=lambda: controller(img_number + 1))
    ...
    if img_number == len(file_list) - 1: #If index equals the size of file_list list...
        btn_next = Button(root, text=">>", padx=btn_padx, pady=btn_pady, fg=fg_color, bg=bg_color,
                          activebackground=activebg_color, activeforeground=fg_color, font=((button_font)),
                          state=DISABLED) #Disable the "Next" button

    elif img_number == 0: #If index equals 0...
        btn_back = Button(root, text="<<", padx=btn_padx, pady=btn_pady, fg=fg_color, bg=bg_color,
                          activebackground=activebg_color, activeforeground=fg_color, font=((button_font)),
                          state=DISABLED) #Disable the "Back" button
